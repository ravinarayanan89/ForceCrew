...
ForceCrew is an open-source framework that brings the [crewAI](https://github.com/crewAIInc/crewAI) programming model to the Salesforce platform. Build teams of collaborating AI agents powered by the **Salesforce Models API** — no HTTP callouts, no Named Credentials, no external dependencies.
...
git clone https://github.com/your-org/forcecrew.git
cd forcecrew
...
new FC_Crew.Builder('My Hierarchical Crew')
    ...
    .managerLlmModel(FC_ModelsAPIService.MODEL_GPT4O)
    .agent(researcher)
    .agent(analyst)
    .agent(writer)
    .task(gatherDataTask)
...
for (FC_Logger.LogEntry entry : entries) {
    ...
    // entry.message
    // entry.timestamp — Datetime.now().getTime()
...
| `MODEL_CLAUDE` | `sfdc_ai__DefaultClaude` |
...
3. Follow the `FC_` prefix and Builder pattern conventions
